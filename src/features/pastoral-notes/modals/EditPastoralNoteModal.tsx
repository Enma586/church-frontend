...
interface Props { open: boolean; onOpenChange: (open: boolean) => void; note: PastoralNote; }
...
export function EditPastoralNoteModal({ open, onOpenChange, note }: Props) {
  ...
  const form = useForm({
    ...
    defaultValues: { content: note.content, isSensitive: note.isSensitive },
  });
  ...
  const onSubmit = (values: z.infer<typeof editSchema>) => {
    ...
    updateMutation.mutate(
      ...
      { onSuccess: () => { notifyUpdated('Nota pastoral'); onOpenChange(false); } },
    );
  };

  return (
    <FormModal open={open} onOpenChange={onOpenChange} title="Editar nota pastoral" size="lg">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-6">
          <div className="bg-muted/30 p-4 rounded-lg border space-y-4">
            <FormTextArea name="content" control={form.control} label="Contenido" rows={5} />
            <div className="flex items-center gap-2">
              <Checkbox id="edit-sensitive" checked={form.watch('isSensitive')} onCheckedChange={(c) => form.setValue('isSensitive', c === true)} />
              <Label htmlFor="edit-sensitive" className="text-sm cursor-pointer">Nota sensible</Label>
            </div>
          </div>
